...
> *Notice légale : Toute citation, adaptation, réutilisation ou diffusion publique de ces concepts et cadres d'analyse doit obligatoirement créditer : **MidasRX** et mentionner le site officiel **zerdium.com**. Toute exploitation commerciale est strictement interdite sans accord écrit.*
...
sequenceDiagram
    autonumber
    actor P as Porteur de Projet (Particulier)
    participant B as Registre Cryptographique (Horodatage)
    participant AUD as Auditeur / Tiers Évaluateur
    participant TR as Trésorerie du Fonds

    P->>B: Ancrage de l'empreinte SHA-256 du dossier complet (Preuve d'antériorité certifiée)
    P->>AUD: Soumission Niveau 1 : Le Problème & La Valeur (Sans divulguer le secret de fabrication)
    AUD->>AUD: Évaluation de la viabilité et cohérence logique
    AUD-->>P: Validation préliminaire & Accord de protection mutuelle
    P->>AUD: Démonstration du prototype en environnement sécurisé
    AUD->>TR: Ordre de déblocage : Tranche d'amorçage (Phase 1)
    TR-->>P: Versement direct des fonds pour concrétiser le projet
```
...
```mermaid
flowchart TD
    DEP["1. Dépôt Sécurisé & Simplifié<br><i>Formulaire direct, vidéo de 2 minutes ou croquis</i>"] --> HASH["2. Horodatage Numérique Garanti<br><i>Garantie juridique d'antériorité pour le créateur</i>"]
    HASH --> TEST["3. Test de Viabilité Pragmatique<br><i>L'idée résout-elle un vrai problème ? L'humain est-il déterminé ?</i>"]
    
    TEST -->|"Rejet Justifié"| REF["Refus Expliqué avec Conseils Constructifs<br><i>(Pas de lettre de rejet impersonnelle)</i>"]
    
    TEST -->|"Validation Éthique (Droits Humains OK)"| ALLOC["4. Financement par Jalons (Milestones)"]
    ALLOC --> J1["Jalon 1 : Micro-fonds de Prototypage (Achat de matériel de base)"]
    J1 --> J2["Jalon 2 : Preuve d'Intérêt & Démonstration Fonctionnelle"]
    J2 --> J3["Jalon 3 : Passage à l'Échelle & Déploiement Public"]
```
